	...
	<div class="shade" v-show="flag">
	 <div class="sku">
	       ...
	       <div class="particulars">
	       		<p>版本</p>
	       	<div class="detail" v-for="(item,index) in edition" @click="detailfn(item,item.color,index)" :class="{active:num==index}">
	       			<p>{{ item.Memory }}</p>
	       			<span>{{ item.edition_price }}</span>
	       		</div>
	       		<div class="color">
	       			<p>颜色</p>
	       			<div class="colortext" v-for="(item,index) in color" @click="detailfn1(item,index)" :class="{active:num1==index}">
	       				<p>{{ item.color_list }}</p>
	       			</div>
	       		</div>
	       </div>
